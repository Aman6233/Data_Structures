...
def searchElements(blockHead, k):
	#find the block
	j = (k+blockSize-1)//blockSize 		# kth element is in jth block
	p = blockHead
	j -= 1
	while j:
		p = p.next
...
def shift(A):
	...
	while (A.nodeCount > blockSize):		# if this block still have to shift
		if (A.next == None):				# if reached end
			A.next = newLinkedBlock()
			B = A.next
			temp = A.head.next				#last element (direction is right-to-left see diagram in book)
...
if __name__ == '__main__':
	...
	print(searchElement(blockHead, 1))
